...
class nipt(object):
	...
	def setdir(self):
		...
		unmapped_bam_file = './unmapped/unmapped_'+self.fq+'.bam'
		remap_bam_file = './remap/remap_'+self.fq+'.bam'
		micro_sam_file = './micro/micro_'+self.fq+'.sam'
		vcf_file = './vcf/snp_' + self.fq + '.vcf'
		a = [input_file,bam_file,sort_bam_file,sort_bam_file_1,rmdup_sort_bam_file,counts_file,unmapped_bam_file,remap_bam_file,micro_sam_file,vcf_file]
		return a
		
	def mapping(self,zuse = 0):
		a = self.setdir()
		p = subprocess.Popen(["subread-align -t 1 -T 40 -M 1 -I 0 -i ./fa/ucsc_hg19/ucsc_hg19 -r"+a[0]+" -o"+a[1]],shell=True) #mapping
		#p = subprocess.Popen(["subread-align -t 1 -T 40 -M 1 -I 1 -i ./fa/tox/tox -r"+a[0]+" -o"+a[1]],shell=True) #mapping
		if zuse == 1:
			p.wait()
		return 
	...
	def unmap(self):
		a = self.setdir()
		p = subprocess.Popen(["samtools view -b -f 4 "+a[4]+" > "+a[6]],shell=True)
		
	def remap(self):
		a = self.setdir()
		p = subprocess.Popen(["subread-align -t 1 -T 40 -M 2 -I 2 --BAMinput -i ./fa/hbv/hbv -r"+a[6]+" -o"+a[7]],shell=True) #mapping
  
	def micro(self):
		a = self.setdir()
		p = subprocess.Popen(["samtools view -f 2 "+a[7]+" > "+a[8]],shell=True)
	
	def flagstat(self):
		a = self.setdir()
		p = subprocess.Popen("samtools flagstat "+a[8],shell=True) 
	...
	def call_snp(self):
		a = self.setdir()
		p = subprocess.Popen('exactSNP -b -Q 1 -T 4 -i '+a[4]+' -g ./fa/fasta/ucsc.hg19.fasta -o '+ a[9],shell=True) 
		#p4.wait()
		#os.remove(a[4])
		return  
...
def main():
	...
	#i = datetime.datetime.now()
	#map_time = str(i)[0:19]
	#onebyone(fq_name,file_dir,0,0,100)
	#i = datetime.datetime.now()
	#sort_time = str(i)[0:19]
	#onebyone(fq_name,file_dir,1,7,30)
	#i = datetime.datetime.now()
	#rmdup_time = str(i)[0:19]
	#onebyone(fq_name,file_dir,2,20,120)
	#onebyone(fq_name,file_dir,4,20,120)
	#i = datetime.datetime.now()
	#counts_time = str(i)[0:19]
	#onebyone(fq_name,file_dir,5,17,0)
	#onebyone(fq_name,file_dir,6,18,0)
	#onebyone(fq_name,file_dir,7,10,0)
	onebyone(fq_name,file_dir,8,25,0)
	#i = datetime.datetime.now()
