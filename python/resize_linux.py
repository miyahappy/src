...
a = os.listdir('./model1/chr')
for i in a:
	img = cv2.imread('./model1/chr/'+i)
	sp = img.shape
	...
	if h >= w:
		...
		cv2.imwrite('./working/chr/'+i,res)
	
a = os.listdir('./model1/num')
for i in a:
	img = cv2.imread('./model1/num/'+i)
	sp = img.shape
	h = sp[0]  # height(rows) of image
	w = sp[1]  # width(colums) of image
	if h >= w :
		...
		out = cv2.cvtColor(out, cv2.COLOR_RGB2GRAY)
		res=cv2.resize(out,(300,300),interpolation=cv2.INTER_CUBIC)
		cv2.imwrite('./working/num/'+i,res)	
a = os.listdir('./test/chr')
for i in a:
	img = cv2.imread('./test/chr/'+i)
	sp = img.shape
	h = sp[0]  # height(rows) of image
	w = sp[1]  # width(colums) of image
	if h >= w :
		...
		out = cv2.cvtColor(out, cv2.COLOR_RGB2GRAY)
		res=cv2.resize(out,(300,300),interpolation=cv2.INTER_CUBIC)
		cv2.imwrite('./test_working/chr/'+i,res)
a = os.listdir('./test/num')
for i in a:	
	img = cv2.imread('./test/num/'+i)
	sp = img.shape
	h = sp[0]  # height(rows) of image
	w = sp[1]  # width(colums) of image
	if h >= w :
		...
		out = cv2.cvtColor(out, cv2.COLOR_RGB2GRAY)
		res=cv2.resize(out,(300,300),interpolation=cv2.INTER_CUBIC)
		cv2.imwrite('./test_working/num/'+i,res)	
...
model = Sequential()
model.add(Conv2D(32,kernel_size=3,activation='relu',input_shape=(300,300,1)))
...
model.compile(loss='sparse_categorical_crossentropy',
              ...
              optimizer=sgd,
              metrics=['accuracy']) 
model.fit_generator(train_generator, steps_per_epoch=20, epochs=15,
					verbose=1)#,validation_data=test_generator,
					#validation_steps=20)
model.save('num_or_chr.h5')
